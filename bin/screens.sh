...
NAME="$(xrandr | grep -Fv VIRTUAL | grep -F connected | fgrep -Fv 'not connected' | cut -d\  -f1 | grep -Fv eDP | head -n 1)"
NAME=${NAME:-HDMI1}
echo "NAME=$NAME"
case "${N:-1}" in
  1) CMD="xrandr --output $NAME --off --output '$nm' --mode $sz --scale $scale --pos 0x0 --rotate normal" ;;
  2) CMD="xrandr --output $NAME --mode $sz --pos ${pos}x0 --rotate normal --output $nm --mode $sz --scale $scale --pos 0x0 --rotate normal" ;;
